...
public class DashController implements Initializable {
    ...
    public void logout(ActionEvent event) {
        try {
            if (event.getSource() == logout_btn || event.getSource() == semiLogout_btn) {
                Parent root = FXMLLoader.load(getClass().getResource("/sample/LoginManager.fxml"));
    ...
    private void loadPieChartData() {
        categoryPieChart.getData().clear(); // Xóa dữ liệu cũ
    ...
    private void showMemberPhoneNumber(int memberCode) {
        String phoneNumber = "";
        try (Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/LibraryDB", "root", "huy2206");
